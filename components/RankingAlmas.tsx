...
export default function RankingAlmas({ editable = false }: { editable?: boolean }) {
  ...
  const handleSaveGoal = async (churchId: string) => {
    ...
    if (churchId === 'GLOBAL') {
      ...
      const church = dbChurches.find(c => c.id === churchId);
      if (church) {
        let config: any = {};
