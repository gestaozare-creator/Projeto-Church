...
function fixDept(file, deptName) {
  ...
  const removeErrStr = 'console.error("Erro ao remover escala do banco"';
  let removeErrIdx = content.indexOf(removeErrStr);
  if (assignStartIdx === -1 || removeErrIdx === -1) {
    console.log("Could not find blocks in " + file);
    return;
  }
  let blockEndIdx = content.indexOf('};', removeErrIdx) + 2;
...
console.log('Fixed exactly as intended, version 2.');
