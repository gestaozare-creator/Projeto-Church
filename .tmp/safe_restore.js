...
const logicReplacement = `  // --- Gráficos de Linha Restabelecidos ---
  const comparativeData = useMemo(() => {
    const data = months.map(m => ({ month: m, year1: null as number | null, year2: null as number | null, members: null as number | null, visitors: null as number | null }));
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
            ...
            <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><div style={{ width: '12px', height: '12px', background: 'var(--primary-light)', borderRadius: '3px' }} /><select value={cmpYear1} onChange={e => setCmpYear1(e.target.value)} className="filter-select" style={{ border: 'none', background: 'transparent', color: '#fff', fontSize: '0.8rem', padding: '0 4px' }}><option value="2026">2026</option><option value="2025">2025</option><option value="2024">2024</option></select></div>
                <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>vs</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><div style={{ width: '12px', height: '12px', background: '#95a5a6', borderRadius: '3px' }} /><select value={cmpYear2} onChange={e => setCmpYear2(e.target.value)} className="filter-select" style={{ border: 'none', background: 'transparent', color: '#fff', fontSize: '0.8rem', padding: '0 4px' }}><option value="2026">2026</option><option value="2025">2025</option><option value="2024">2024</option></select></div>
              </div>
            </div>
          </div>
          <div style={{ position: 'relative', height: '150px', display: 'flex', flexDirection: 'column' }}>
            <div style={{ position: 'relative', height: '150px', flexShrink: 0 }}>
              ...
              <div style={{ position: 'absolute', inset: 0, display: 'flex' }}>
                {comparativeData.map((d, i) => { const isHovered = hoveredMonthIdx === i; return <div key={i} style={{ flex: 1, position: 'relative', cursor: 'crosshair' }} onMouseEnter={() => setHoveredMonthIdx(i)} onMouseLeave={() => setHoveredMonthIdx(null)}>{isHovered && <div style={{ position: 'absolute', left: '50%', top: 0, bottom: 0, width: '1px', background: 'rgba(255,255,255,0.1)', transform: 'translateX(-50%)', pointerEvents: 'none' }} />}</div>; })}
              </div>
                  ...
                  <div key={\`dots1-\${i}\`} style={{ position: 'absolute', left, top: 0, width: 0, height: '100%', pointerEvents: 'none' }}>
                    ...
                    {isHovered && (
                      <div style={{ position: 'absolute', top: '-40px', left: '50%', transform: 'translateX(-50%)', background: 'rgba(20,20,30,0.95)', border: '1px solid rgba(255,255,255,0.1)', padding: '8px 12px', borderRadius: '8px', zIndex: 10, display: 'flex', flexDirection: 'column', gap: '4px', whiteSpace: 'nowrap', boxShadow: '0 4px 12px rgba(0,0,0,0.5)', pointerEvents: 'none' }}>
                        ...
                        {d.year1 !== null && <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.75rem' }}><div style={{ width: '8px', height: '8px', background: 'var(--primary-light)', borderRadius: '2px' }} /><span style={{ color: '#fff' }}>{cmpYear1}:</span> <strong>{d.year1}</strong></div>}
                        {d.year2 !== null && <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.75rem' }}><div style={{ width: '8px', height: '8px', background: '#95a5a6', borderRadius: '2px' }} /><span style={{ color: '#fff' }}>{cmpYear2}:</span> <strong>{d.year2}</strong></div>}
            ...
            <div style={{ display: 'flex', marginTop: '10px' }}>
              {comparativeData.map((d, i) => <div key={i} style={{ flex: 1, textAlign: 'center', fontSize: '0.65rem', color: 'var(--text-secondary)' }}>{d.month}</div>)}
            </div>
          </div>
        </div>
        ...
        <div className="glass" style={{ padding: '20px', borderRadius: '14px', display: 'flex', flexDirection: 'column', minHeight: '300px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
            ...
            <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><div style={{ width: '12px', height: '12px', background: '#2ecc71', borderRadius: '3px' }} /><span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Membros</span></div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><div style={{ width: '12px', height: '12px', background: '#f39c12', borderRadius: '3px' }} /><span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Visitantes</span></div>
              </div>
            </div>
          </div>
          <div style={{ position: 'relative', height: '150px', display: 'flex', flexDirection: 'column' }}>
                ...
                return (
                  <div key={\`dots2-\${i}\`} style={{ position: 'absolute', left, top: 0, width: 0, height: '100%', pointerEvents: 'none' }}>
                    ...
                    {isHovered && (
                      <div style={{ position: 'absolute', top: '-40px', left: '50%', transform: 'translateX(-50%)', background: 'rgba(20,20,30,0.95)', border: '1px solid rgba(255,255,255,0.1)', padding: '8px 12px', borderRadius: '8px', zIndex: 10, display: 'flex', flexDirection: 'column', gap: '4px', whiteSpace: 'nowrap', boxShadow: '0 4px 12px rgba(0,0,0,0.5)', pointerEvents: 'none' }}>
                        <div style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', fontWeight: 600, textAlign: 'center', marginBottom: '2px' }}>{d.month}</div>
                        {d.members !== null && <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.75rem' }}><div style={{ width: '8px', height: '8px', background: '#2ecc71', borderRadius: '2px' }} /><span style={{ color: '#fff' }}>Membros:</span> <strong>{d.members}</strong></div>}
                        {d.visitors !== null && <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.75rem' }}><div style={{ width: '8px', height: '8px', background: '#f39c12', borderRadius: '2px' }} /><span style={{ color: '#fff' }}>Visitantes:</span> <strong>{d.visitors}</strong></div>}
...
// Logic replacement
const logicStart = curContent.indexOf('// --- Gráfico Comparativo ---');
const logicEnd = curContent.indexOf('// --- Gráfico Rosca Visitantes ---');
if (logicStart !== -1 && logicEnd !== -1) {
  curContent = curContent.substring(0, logicStart) + logicReplacement + curContent.substring(logicEnd);
}

// JSX replacement
const jsxStart = curContent.indexOf('{/* COMPARAÇÃO DE CRESCIMENTO & ROSCA DE VISITANTES */}');
if (jsxStart !== -1) {
  curContent = curContent.substring(0, jsxStart) + jsxReplacement;
}

// Ensure "months" is declared before logicReplacement
if (curContent.indexOf("const months = ['Jan'") === -1) {
  curContent = curContent.replace('const comparativeData = useMemo(() => {', "const months = ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'];\n  const comparativeData = useMemo(() => {");
}
