...
export default function LouvorDashboardPage() {
  ...
  const [timelineDates, setTimelineDates] = useState<string[]>(
    getCultosDoMes(selectedMonthStr),
  );
  const [activeDate, setActiveDate] = useState<string>(
    timelineDates[0] || new Date().toISOString().split("T")[0],
  );

  // Record<date, Record<role, memberId[]>>
  const [escalasGlobais, setEscalasGlobais] = useState<
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (
        ...
        churchDb.config.escalas["Louvor"]
      ) {
        setEscalasGlobais(churchDb.config.escalas["Louvor"]);
      } else {
        setEscalasGlobais({});
      }
    }
  ...
  const saveToConfig = async (newEscalas: any) => {
    ...
    currentConfig.escalas["Louvor"] = newEscalas;
    await supabase
      .from("churches")
  ...
  const stats = useMemo(() => {
    ...
    const porFuncao = dbMembers.reduce(
      (acc, m) => {
  ...
  const participacoesMes = useMemo(() => {
    ...
    return Object.entries(contagem)
      .map(([id, count]) => ({
        member: dbMembers.find((m) => m.id === id),
        count,
      }))
    ...
    >
      ...
      <div className="header" style={{ marginBottom: "10px" }}>
        <div>
          ...
          <span style={{ fontSize: "0.85rem", color: "var(--text-secondary)" }}>
            Análise da equipe e Gestão de Escalas Musicais
          </span>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: "0.75rem", color: "#2ecc71" }}>
            {stats.ativos} Ativos /{" "}
            <span style={{ color: "#e74c3c" }}>{stats.inativos} Inativos</span>
          </div>
        </div>
        ...
        >
          ...
          >
            Top Participações (Mês:{" "}
            {selectedMonthStr.split("-").reverse().join("/")})
          </div>
          {participacoesMes.length === 0 ? (
            ...
            >
              Nenhuma escala computada neste mês ainda.
            </div>
          ) : (
            <div
              style={{
      ...
      <div
        style={{
          display: "grid",
          gridTemplateColumns:
            "minmax(240px, 1fr) minmax(320px, 2fr) minmax(280px, 1.2fr)",
          gap: "20px",
                    ...
                    {hasScale && (
                      <div
                        style={{
                          ...
                          height: "4px",
                          borderRadius: "50%",
                          background: isActive ? "#fff" : "#2ecc71",
