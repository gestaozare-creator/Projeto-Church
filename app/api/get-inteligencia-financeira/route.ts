...
export async function GET(req: Request) {
  try {
    ...
    const prevMonthFormatted = (prevMonthNum + 1).toString().padStart(2, '0');
    const startPrevStr = `${prevYearNum}-${prevMonthFormatted}-01`;
    const lastDayPrev = new Date(yearNum, monthNum, 0).getDate();
    const endPrevStr = `${prevYearNum}-${prevMonthFormatted}-${lastDayPrev.toString().padStart(2, '0')}`;
    ...
    if (!churches || churches.length === 0) {
      return NextResponse.json({ success: true, churchesData: [], globalData: {} });
    }

    const churchIds = churches.map(c => c.id);

    // Fetch transactions for the previous month (only paid incomes to calculate MoM growth)
    const { data: prevTransactions } = await supabaseAdmin
      .from('transactions')
      .select('amount, type, church_id')
    ...
    const { data: currentTransactions } = await supabaseAdmin
      .from('transactions')
    ...
    let globalReceita = 0;
    let globalDespesa = 0;
    let globalReceitaPrev = 0;
    let globalEntradasCount = 0;

    const churchesData = churches.map(church => {
      const cId = church.id;
      const tPrev = (prevTransactions || []).filter(t => t.church_id === cId);
      const tCurr = (currentTransactions || []).filter(t => t.church_id === cId);

      let receitaAnterior = 0;
      tPrev.forEach(t => { receitaAnterior += Number(t.amount); });
