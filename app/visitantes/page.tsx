...
export default function Visitantes() {
  ...
  const groupedList = useMemo(() => {
    ...
    filtered.forEach(v => {
      ...
      const monthStr = new Date(v.date + 'T12:00').toLocaleDateString('pt-BR', { month: 'long' });
      const month = monthStr.charAt(0).toUpperCase() + monthStr.slice(1);
      const day = v.date.split('-')[2];
      const culto = v.culto || 'Geral';
      const horario = v.horario || 'Sem horário';
      const key = `${year}##${month}##${day}##${culto}##${horario}##${v.date}`;
      if (!groups[key]) groups[key] = [];
      groups[key].push(v);
    });
    return Object.keys(groups).sort((a, b) => b.localeCompare(a)).map(key => {
      const [year, month, day, culto, horario, fullDate] = key.split('##');
          ...
          <div key={group.key} style={{ marginBottom: '24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '12px' }}>
              {group.visitors.map(v => {
                const isSel = sel?.id === v.id;
                return (
                  <div key={v.id} className={`glass member-card ${isSel ? 'card-selected' : ''}`} style={{ padding: '14px 16px', flexDirection: 'row', alignItems: 'center', gap: '14px', cursor: 'pointer' }} onClick={() => setSel(v)}>
                    <div style={{ width: '45px', height: '45px', borderRadius: '50%', background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: '700', fontSize: '1rem', flexShrink: 0, border: '2px solid rgba(255,255,255,0.1)' }}>
                      {v.name.split(' ').map(n => n[0]).join('').slice(0, 2)}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <span style={{ fontWeight: '600', fontSize: '0.95rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{v.name}</span>
                        {statusBadge(v.status)}
                        {v.wantsVisit && v.status !== 'membro' && <span style={{ fontSize: '0.55rem', padding: '1px 4px', borderRadius: '4px', background: 'rgba(231,76,60,0.15)', color: '#e74c3c', fontWeight: '700', border: '1px solid rgba(231,76,60,0.3)' }}>VISITA</span>}
                      </div>
